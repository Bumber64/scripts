...
function HFfindWindow:onRenderFrame(dc, rect)
...
function HFfindWindow:onResizeEnd(ok, frame) --Adjust list width
    self.subviews.sel_list.frame.w = frame.w-35
    self.subviews.sel_list:updateLayout()
end

function HFfindWindow:on_submit_choice(sel, obj)
    self.target_hf = obj and obj[2] or nil
end

function HFfindWindow:on_edit_change(txt) --Fill choices from search
